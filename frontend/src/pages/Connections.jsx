...
import { dummyFollowersData  as followers,
...
{/* Title */}
        <div className="mb-8">
          ...
          <p className="text-slate-600">
            Manage your network and discover new connections </p>
        </div>
...
<div key={index} className="flex flex-col items-center justify-center gap-1 border h-20 w-40 border-gary-200 bg-white shadow rounded-md">
...
{/* tabs */}
        <div className="inline-flex flex-wrap items-center border border-gary-200 rounded-md p-1 bg-white shadow-sm">
          {
            dataArray.map((tab)=>(
              <button onClick={()=> setCurrentTab(tab.label)}
              key={tab.label} className=
              {`flex items-center px-3 py-1
              cursor-pointer text-sm rounded-md transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
                <tab.icon className="w-4h-4"/>
                <span className="ml-1">{tab.label}</span>
                {
                  tab.count !== undefined  && (
                    <span className="ml-2 text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full">{Tangent.count}</span>
                  )
                }
              </button>
            ))
...
View profile
  ...
  <button
    ...
    className="w-full flex items-center gap-2 p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-black active:scale-95 transition cursor-pointer"
  >
